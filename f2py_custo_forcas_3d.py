...
N_min, N_max, qnt_em_qnt = 50, 1000, 50
qntd_testes_N = 20
eps = 1e-2
G = 1.0
...
for N in range(N_min, N_max + 1, qnt_em_qnt):
  ...
  for _ in range(qntd_testes_N):
    ...
    qs = 10. * (2. * np.random.random((N,3)) - 1.)
    x, y, z = np.array(list(zip(*qs)))
    
    # para o calculo direto
    forcas0 = np.zeros((N,3))
    # iniciando o timer
    tempo_0 = time()
    # calcula
    forcas0 = api.api_mod.compute_forces_direct(massas, x, y, z, G, eps)
    # encerra o timer
    tempo = time() - tempo_0
    ...
    for i, theta in enumerate(thetas):
      ...
      forcas = api.api_mod.compute_forces(massas, x, y, z, theta, G, eps, True)
...
plt.grid(True)
plt.savefig("img/f2py_3d_custo_forcas_theta.png")
plt.close()
...
plt.grid(True)
plt.ylim(1e-5, 1e-0)
plt.savefig("img/f2py_3d_erro_forcas_theta.png")
plt.close()