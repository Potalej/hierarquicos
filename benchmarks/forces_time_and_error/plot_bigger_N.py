...
for i, title in enumerate(files):
    ...
    thetas_uniques = [0.25, 0.5, 0.75, 1.0, -1]
    grouped = {}
    ...
    for i_theta, theta in enumerate(thetas_uniques):
        ...
        for test in range(num_tests):
            x_axis.append(Ns[test*len(Ns)//num_tests])
    ...
    fig.suptitle(f"{title} ({num_threads} threads)", fontsize=16)
    plt.savefig(f"{out_folder}/times_BIG_{title}_nt{num_threads}.png", dpi=200)
    plt.close()