...
TYPE :: OctreeType
    ...
    REAL(16) :: side_amplificator = 1.2d0

    ! to save_txt
    INTEGER :: save_txt

    ! masses, positions and number of bodies (N)
    REAL(16), ALLOCATABLE :: m(:), x(:), y(:), z(:)
    INTEGER :: N

    ! nodes
    INTEGER :: number_of_nodes = 0
    INTEGER :: max_number_of_nodes
    REAL(16), ALLOCATABLE :: ns_cx(:), ns_cy(:), ns_cz(:) ! centers
    REAL(16), ALLOCATABLE :: ns_halfside(:), ns_L2(:)
    REAL(16), ALLOCATABLE :: ns_mass(:), ns_qcm_x(:), ns_qcm_y(:), ns_qcm_z(:)
    REAL(16), ALLOCATABLE :: ns_quad(:,:)
...
! @calledby init, allocate_subnode
    CLASS(OctreeType), INTENT(INOUT) :: self
    REAL(16), INTENT(IN) :: cx, cy, cz, side
...
! @calledby add_to_subnode
    ...
    REAL(16) :: h, h_half, cx, cy, cz, cx_sub, cy_sub, cz_sub
...
SUBROUTINE evaluate_quad_aprox (self, par_node_idx)
    ...
    REAL(16) :: pm, px, py, pz
    REAL(16) :: dxi, dyi, dzi ! quadrupole
...
! to evaluate the forces in the tree.
    CLASS(OctreeType), INTENT(INOUT) :: self
    INTEGER, INTENT(IN) :: p ! particle index
    REAL(16), INTENT(IN), OPTIONAL :: par_theta2, par_G, par_eps ! parameters
    REAL(16) :: eps, theta2, G

    REAL(16) :: pm, px, py, pz
    REAL(16) :: forces(3)
    REAL(16) :: dx, dy, dz, dist2, L2, f

    INTEGER :: stack(self % number_of_nodes)
    INTEGER :: top, node_idx, i, child_idx

    REAL(16) :: rinv, invR, invR3, invR5
    REAL(16) :: Mx, My, Mz
    REAL(16) :: Qx, Qy, Qz
    REAL(16) :: mxi2, myi2, mzi2, mxyi, mxzi, myzi

    REAL(16) :: dx2, dy2, dz2, dxyz, invR2
